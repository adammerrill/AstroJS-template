...
function mapZodType(field: SchemaField): string {
  ...
  switch (field.type) {
    ...
    case 'multilink':
      ...
      zodType = 'z.object({ cached_url: z.string().optional(), url: z.string().optional(), linktype: z.enum(["url", "story", "email", "asset"]).optional() })';
      break;
    case 'richtext':
    ...
    case 'bloks':
      ...
      if (field.component_whitelist) {
        ...
        if (whitelist.length > 0) {
          ...
          const schemaNames = whitelist.map((name: string) => `${toPascalCase(name)}BlokSchema`);
