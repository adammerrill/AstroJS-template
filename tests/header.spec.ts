...
test.describe('Header Component - Production Validation', () => {
  ...
  test.describe('Breakpoint Behavior (1020px)', () => {
    test('Wide: ≥1020px hides hamburger and shows desktop nav', async ({ page }) => {
      await page.setViewportSize(viewports.wide);
      await page.goto('/');
      
      await expect(page.locator('#mobile-menu-button')).toBeHidden();
      await expect(page.locator('[data-testid="desktop-nav"]')).toBeVisible();
      await expect(page.locator('span:text-is("Astro Template")')).toBeVisible();
      
      // Only visible CTA (not drawer CTA)
      await expect(page.locator('[data-testid="header-cta"] >> visible=true')).toBeVisible();
    });

    test('Narrow: <1020px shows hamburger and hides desktop nav', async ({ page }) => {
    ...
    test('Breakpoint minus 1px (1019px): hamburger visible', async ({ page }) => {
  ...
  test.describe('Layout & Spacing', () => {
    ...
    test('Max width: 1280px', async ({ page }) => {
      ...
      await page.goto('/');
      await expect(page.locator('#main-header > div')).toHaveCSS('max-width', '1280px');
    });
    ...
    test('Right alignment: CTA/hamburger at right edge', async ({ page }) => {
      ...
      if (groupBox && containerBox) {
        expect(groupBox.x + groupBox.width).toBe(containerBox.x + containerBox.width - 16);
      }
    });
  ...
  test.describe('Mobile Drawer', () => {
    ...
    test('Contains navigation and footer links', async ({ page }) => {
      ...
      await expect(page.locator('#mobile-drawer a:text-is("Contact")')).toBeVisible();
    });
  ...
  test.describe('CTA Persistence', () => {
    ...
    test('CTA appears once in header, once in drawer when open', async ({ page }) => {
      await page.setViewportSize(viewports.narrow);
      await page.goto('/');
      
      // Initially one VISIBLE CTA in header (drawer CTA is hidden)
      const visibleCtas = page.locator('[data-testid="header-cta"] >> visible=true');
      ...
      const headerCta = page.locator('[data-testid="header-cta"] >> visible=true');
      const drawerCta = page.locator('[data-testid="drawer-cta"] >> visible=true');
