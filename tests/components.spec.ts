...
test.describe("Component Architecture", () => {
  ...
  test("Svelte 5 Button primitive handles events", async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      btn.className = 'inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium btn-primary h-10 px-4 py-2';
      btn.textContent = 'Click Me';
      btn.onclick = () => { btn.dataset.clicked = 'true'; };
