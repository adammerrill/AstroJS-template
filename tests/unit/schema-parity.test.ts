...
describe("Pipeline Integrity: Schema-Mock Parity", () => {
  const components = Object.keys(MockFactory) as Array<
    keyof typeof MockFactory
  >;
  ...
  it.each(components)("Mock for %s satisfies Zod Schema", (componentKey) => {
    ...
    if (!schema) {
      throw new Error(
        `Schema not found for ${schemaName}. Check naming consistency.`,
      );
    }
    ...
    if (!result.success) {
      console.error(
        `Validation failed for ${componentKey}:`,
        result.error.format(),
      );
    }
